...
import { addPointSiteRedemption, deletePointSiteRedemption, getUserPointSiteRedemptions } from '@/lib/firestore';
import type { PointSiteRedemption } from '@/types';

const SITE_SUGGESTIONS = ['モッピー', 'ハピタス', 'ポイントインカム', 'その他'];
...
export function PointSiteRedemptionManager({ userId }: PointSiteRedemptionManagerProps) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ siteName: '', amount: '', redeemedAt: new Date().toISOString().split('T')[0], memo: '' });
      setShowForm(false);
      await load();
    } catch {
      setError('保存に失敗しました');
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="glass-panel p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-800">還元を記録</h3>
